        ...
        <div class="textCenter">
            <div class="imagesUploadBlock">
                <label class="uploadImg" for="image-{{ $name }}">
                    ...
                    <input type="file"  {{ $required ? 'required' : '' }} {{ $disabled ? 'disabled' : '' }} @foreach($parameters as $key => $parameter) {{ $key . '=' . $parameter }} @endforeach  name="{{ $name }}" id="image-{{ $name }}" class="imageUploader">
                </label>
                @if($src)
